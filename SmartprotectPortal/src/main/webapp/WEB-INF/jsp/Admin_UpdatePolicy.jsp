        ...
        <div class="col-lg-12">            
          <div class="card">
            <div class="card-body">            
              ...
              <form:form action="savePolicyUpdate" method="post" modelAttribute="p" enctype="multipart/form-data">
              <form:hidden path="policyId"/>
              	<table class="table table-borderless">              			
                        <tr>
                        <td>Policy Category</td>
                        <td>
                        <form:select class="form-control" path="policyCategory">
                    		 <c:forEach items="${catList}" var="c">
        							<c:if test="${c.catId eq p.policyCategory.catId}">
  										 <option value="${c.catId}" selected>${c.catName}</option>
									</c:if>
									<c:if test="${c.catId ne p.policyCategory.catId}">
   										<option value="${c.catId}">${c.catName}</option>
									</c:if>
    							</c:forEach>
                  		 </form:select>
                        </td>                  
                    <td><form:errors path="policyCategory" cssClass="error"/></td>
                </tr>
                <tr>
                    <td>Policy Code</td>
                    <td><form:input type="text" path="policyCode"/></td>                  
                    <td><form:errors path="policyCode" cssClass="error"/></td>
                </tr>
                <tr>
                    <td>Policy Name</td>
                    <td><form:input type="text" path="policyName"/></td>                  
                    <td><form:errors path="policyName" cssClass="error"/></td>
                </tr>
                <tr>
                    <td>Description</td>
                    <td><a href="#" target="_blank">${p.policyDocpath}</a><input type="file" class="form-control" name="file1" accept="application/pdf"></td>                  
                    <td><form:errors path="policyDocpath" cssClass="error"/></td>
                </tr>
                <tr>
                    <td>Sum assurance</td>
                    <td><form:input type="text" path="policySumassurance"/></td> 
                    <td><form:errors path="policySumassurance" cssClass="error"/></td>
                </tr>
                <tr>
                    <td>Premium(Per Month)</td>
                    <td><form:input type="text" path="policyPremium"/></td>                   
                    <td><form:errors path="policyPremium" cssClass="error"/></td>
                </tr>
                <tr>
                    <td>Tenure</td>
                    <td><form:input type="text" path="policyTenure"/></td>
                    <td><form:errors path="policyTenure" cssClass="error"/></td>
                </tr>
                <tr>
                    <td></td>
                    <td>
